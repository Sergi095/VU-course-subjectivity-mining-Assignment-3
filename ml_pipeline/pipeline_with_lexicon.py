...
def naive_bayes_counts_lex():
    return CombinedFeaturesPipeline(preprocessing.std_prep(), representation.count_vectorizer({'min_df': 1}),
                                    ...
                                    MultinomialNB())
def naive_bayes_counts_lex_bgram(min_df,
                                 ngram_range):
    return CombinedFeaturesPipeline(preprocessing.std_prep(), representation.count_vectorizer({'min_df': min_df,'ngram_range':ngram_range}),
                                    preprocessing.lex_prep(), representation.count_vectorizer({'min_df': min_df,'ngram_range':ngram_range}),
                                    MultinomialNB())
