...
def naive_bayes_counts_bgram(min_df,
                             ngram_range):

    return pipeline(preprocessing.std_prep(),
                    representation.count_vectorizer({'min_df': min_df,'ngram_range': ngram_range}),
                    MultinomialNB())
def naive_bayes_tfidf_bgram(min_df,
                            ngram_range):
    return pipeline(preprocessing.std_prep(),
                    representation.tfidf_vectorizer({'min_df': min_df,'ngram_range': ngram_range}),
                    MultinomialNB())
...
def svm_libsvc_counts_char():
    return pipeline(preprocessing.std_prep(), representation.count_vectorizer({'analyzer':'char' ,'ngram_range': (1,4)}), svm.LinearSVC(max_iter=10000, dual=False, C=0.1))

# including character n-grams ranging from 1 to 4 characters)
